...
TEST_CASE("components can be added to entities", "[ecs]")
{
	...
	SECTION("if there's a component of the same type present, no error is yielded and the returned handle is invalid")
	{
		auto cmp = entity.addComponent<aeyon::Transform>();
		REQUIRE(!cmp);
		REQUIRE(entity.getComponent<aeyon::Transform>() == component);
	}
}
