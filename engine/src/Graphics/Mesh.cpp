...
namespace aeyon
{
    Mesh::Mesh(VertexFormat vertexFormat)
            : m_vertexBuffer(VertexAttribute::fromVertexFormat(vertexFormat)),
    ...
    void Mesh::apply()
    {
        ...
        for (std::size_t vIndex = 0; vIndex < m_positions.size(); vIndex++)
        {
            ...
            if (m_vertexFormat == VertexFormat::P1N1 || m_vertexFormat == VertexFormat::P1N1UV1 ||
                m_vertexFormat == VertexFormat::P1N1UV1T1B1)
            {
                std::memcpy(
    ...
    const std::vector<Mesh::TIndex>& Mesh::getTriangles(std::size_t submesh) const
    {
